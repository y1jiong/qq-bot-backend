...
type (
	CommandReq struct {
		...
		ChatEcho  bool   `json:"chat_echo" description:"回显到对应聊天"`
		Async     bool   `json:"async" description:"提前返回，异步执行命令"`
		Timestamp int64  `json:"timestamp" v:"required" description:"单位：秒；超过 5 秒的请求会被拒绝"`
		Signature string `json:"signature" v:"required" description:"以 token+command+group_id+timestamp+chat_echo+async 为原文，以 token_name 为 key 的 HmacSHA256 值的 base64 值"`
	}
	CommandRes struct {
