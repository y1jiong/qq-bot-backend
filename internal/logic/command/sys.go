...
func trySysGrant(ctx context.Context, cmd string) (catch bool, retMsg string) {
	...
	case doubleValueCmdEndRe.MatchString(cmd):
		...
		case "raw":
			...
			retMsg = service.User().GrantGetRawMsgReturnRes(ctx, gconv.Int64(dv[1]))
			catch = true
		case "namespace":
			// /sys grant <user_id> namespace
			retMsg = service.User().GrantOpNamespaceReturnRes(ctx, gconv.Int64(dv[1]))
			catch = true
		case "token":
			// /sys grant <user_id> token
			retMsg = service.User().GrantOpTokenReturnRes(ctx, gconv.Int64(dv[1]))
			catch = true
		}
	}
	return
...
func trySysRevoke(ctx context.Context, cmd string) (catch bool, retMsg string) {
	...
	case doubleValueCmdEndRe.MatchString(cmd):
		...
		case "raw":
			...
			retMsg = service.User().RevokeGetRawMsgReturnRes(ctx, gconv.Int64(dv[1]))
			catch = true
		case "namespace":
			// /sys revoke <user_id> namespace
			retMsg = service.User().RevokeOpNamespaceReturnRes(ctx, gconv.Int64(dv[1]))
			catch = true
		case "token":
			// /sys revoke <user_id> token
			retMsg = service.User().RevokeOpTokenReturnRes(ctx, gconv.Int64(dv[1]))
			catch = true
		}
	}
	return
