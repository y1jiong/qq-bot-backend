...
func (s *sBot) SendGetWebhook(ctx context.Context, url string) (body string, err error) {
	c := gclient.New()
...
func (s *sBot) SendPostWebhook(ctx context.Context, url string, payload any) (body string, err error) {
	c := gclient.New()
	c.SetAgent(consts.ProjName + "/" + consts.Version)
