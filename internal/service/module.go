...
type (
	IModule interface {
		...
		WebhookGetHeadConnectOptionsTrace(ctx context.Context, method, url string) (statusCode int, body []byte, err error)
		WebhookPostPutPatchDelete(ctx context.Context, method, url string, payload any) (statusCode int, body []byte, err error)
	}
)
