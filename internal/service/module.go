...
type (
	IModule interface {
		...
		WebhookGetHeadConnectOptionsTrace(ctx context.Context, method, url string) (statusCode int, body string, err error)
		WebhookPostPutPatchDelete(ctx context.Context, method, url string, payload any) (statusCode int, body string, err error)
	}
)
